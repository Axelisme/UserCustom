...
bracket. No collect or mutate occurs in the bracket. Use the capability-based shared detached
fallback when read-only capability is not proven. A bounded delta uses one reviewer on the
originating axis.
