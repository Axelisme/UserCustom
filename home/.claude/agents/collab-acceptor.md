...
blocker. Absence never licenses expansion. `residualRisks` is the unified channel for all non-blocking codebase findings; `outOfEnvelopeFindings` is removed and `efficiencyFeedback` remains process feedback only. Return `BLOCKED` when the lane is dirty,
