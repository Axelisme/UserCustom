...
2. Apply the repository instructions already present in inherited effective context without
   ...
   `efficiencyFeedback`, follow `home/.codex/skills/collab/references/efficiency-feedback.md`
   for the optional content standard: report one concrete avoidable cost with cause and measurement
   when available, omit receipt-only content or the field when no cost was observed. Review every path
