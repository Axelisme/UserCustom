...
**A question is not a verdict.** You have no live channel to the Orchestrator: you return once. A
contract contradiction or new-scope question needing Orchestrator judgement is returned as `BLOCKED`
with the question as the blocker's first line, marked as a question rather than a defect, so the
Orchestrator answers it instead of dispatching a correction against it. The question may itself
propose where a seam belongs and what it would carry; the Orchestrator designs and decides.
