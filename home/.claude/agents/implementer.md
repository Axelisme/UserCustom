...
milestone with `state=terminal`, `outcome=validated|review|blocked|needs_decision`, exact
`subject_sha` when clean, evidence, finding IDs, and `next=continue|idle|stop`. The final
