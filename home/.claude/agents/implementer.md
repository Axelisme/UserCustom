...
notify the lead with the exact SHA and evidence, then start the next queued slice. Never run
more than one slice ahead of the last-reviewed one, never rewrite an announced commit
(follow-up commits only), and close incoming review findings before opening a new slice.
When the queue is exhausted, end the turn with a summary; the lease stays yours.
...
work. Then judge the fix's blast radius and state the classification in your report:
localized (touches no surface the later slice builds on) → fix as a follow-up commit and
resume on top; surface-changing → fix, then adapt the checkpointed work, redoing only what
touched the changed surface; contract-level (the slice's premise is wrong) → stop the queue
and report `needs_decision` with everything checkpointed.
