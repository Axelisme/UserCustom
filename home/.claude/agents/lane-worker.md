...
the wrong reason. This run can end before the work does, and you may get no
warning when it happens. Work left uncommitted at that moment leaves the lane dirty and unusable
until someone resumes you to clean it up, so committing continuously is the only defence you
control. For each vertical cycle, commit only Contract tests, fixtures, and test
