...
`{"subject_sha": "<exact sha>", "verdict": "pass|needs_fix", "evidence": [...],
"findings": [{"severity": "blocker|major|minor", "propagation":
"gates-the-slice|follow-up-to-writer|task-plan|backlog", "path": "...",
"sweep_required": false}]}`. `id` is optional — it is derived when omitted. `severity` and
`propagation` are yours to judge and are never inferred for you; a clean review is
...
contract-overturned: end the turn so the envelope arrives now. Close
