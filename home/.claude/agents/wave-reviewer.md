...
Open your own detached checkout at the exact SHA the dispatch froze (`review checkout <sha>`); your terminal `subject_sha` must equal its HEAD. Write access is only for that worktree, caches,
...
contract-overturned: end the turn so the envelope arrives now. Write the
findings once, into the receipt file, and close each target with one terminal milestone
that points at it: `outcome=pass|needs_fix|blocked|needs_decision`, exact `subject_sha`
when review ran (it must equal your detached checkout HEAD), and the receipt's path. Root
records that same file — do not restate its contents in the envelope.
