...
19. **`ask-matt`'s implement/collab routing sentence names no choice at all** — upstream asks the
    reader to pick. This fleet has removed the choice twice over. The sentence first pointed at a
    per-ticket criterion in `dev-flow` that was retired for never firing: across three archived
    tasks, 39 tickets dispatched and not one of them by that criterion. It then named a task-level
    fork — a task owning a managed integration ran its tickets through `collab`, one without worked
    them through `implement` — and that fork was itself removed on 2026-09-02, because a fork
    decided before any ticket existed left the graduation batch with two possible consumers. The
    current sentence states that every ticket goes through `collab` and that collab's own placement
    decides the writer and the reviewer, the Orchestrator writing the change itself being one of
    those placements. It points at `dev-flow`'s lifecycle for the routing and leaves placement to
    collab, rather than pointing at `Jurisdiction`, which no longer carries a routing criterion. The clause offering
