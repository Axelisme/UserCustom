---
name: collab
description: "Caller coordination for one bounded implementation: use when the Caller places a writer, selects a review procedure, resolves a correction handback, collects, or lands."
---

# Collab

Collab coordinates one bounded change without owning a durable lifecycle. The caller retains task
intent, acceptance criteria, final Acceptance judgement, and every scope or authority decision.

**This is the caller's document; the agent profiles are the workers'.** Writer placement, review
placement, collection, and landing are decided here, by the caller. A dispatched writer or reviewer
...
- **Integration branch** — the task-local cumulative result already accepted by the Caller. This is
...
- **Collect** — movement of the Caller-accepted lane subject into integration without introducing
  another subject.
...
1. **One writer at a time per checkout.** This is concurrent exclusivity, not a lifetime binding:
   ...
   second writer. That checkout, with the branch, worktree, and temporary state it owns, is one
   lane. Parallelize read-only work; give each concurrent writer a separate writable checkout.
...
3. **A changed subject voids the pass.** Corrections return to implementation and review as a new
   fixed subject needing its own review result.
...
1. **Bound the change.** Give the writer the goal, write scope, constraints, expected result,
   ...
   distinguish in-scope implementation from a Caller decision and every receiver field is supplied.
2. **Place the writer.** Preserve one live writer per writable checkout and account for pre-existing
   state. Isolate when another writer is active or existing work needs protection. Before
   dispatching, read this skill directory's `runtime-<name>.md` for your runtime when one exists;
   it carries that runtime's dispatch, limit, and resume behaviour. This step is complete when the
   writer has one bounded brief and one safe writable checkout.
3. **Implement and identify.** The writer returns the fixed subject identity, changed paths, the
   ...
   one immutable state.
4. **Review the fixed subject.** The caller selects a review procedure — generic Acceptance below
...
5. **Resolve the result.** The caller makes the final Acceptance judgement and selects the next
   owner. Send a bounded defect for correction; return new scope, product, architecture, or
   authority decisions to the caller. A correction returns to step 3. When the task owns an
   integration branch, an accepted lane subject goes to the collection boundary below before its
   lane retires. This step is complete when the caller accepts the result or identifies the
   unresolved decision and its owner.
6. **Collect.** Move the Caller-accepted lane subject into the integration branch one lane at a
   ...
   Caller judgement. This step is complete when the accepted subject is the integration head and
...
points to; the task record and sibling tickets belong to the caller, and handing them over invites
...
and return a review result; the caller records it where the task's durable judgement lives.
...
the caller owns the final judgement and closure. A fix that needs a new product, scope,
architecture, or authority decision goes back to the caller.
...
single bounded change is a worker → reviewer → bounded correction loop: the caller supplies a
...
to the worker; scope, architecture, authority, or contract decisions terminate back at the caller.
...
decision that is needed, applicable direct observations, and residual risk. Caller- and
user-observed Acceptance items are reported there rather than mislabelled as blockers.

## Collection boundary

When existing work becomes the managed integration, preview `integration_adopt` with `dry_run: true`
before authorizing the mutation. Adoption uses `source_branch` as the complete managed integration
state. It discards changes from that integration when their lane has not completed `lane_collect`.
It leaves any pre-existing lane branch or worktree outside the adopted integration unless a separate
lifecycle operation retires it.

Collection moves an accepted lane subject into the task-local integration branch, one lane at a
time, without introducing another subject. The runtime verifies the exact lane tip, that current
integration is contained in the judged subject, and that managed refs and worktrees are clean and
identity-exact; it then advances integration to the exact subject and retires the lane when it is
clean. A lane that no longer contains current integration is synchronized first and stops at the
new subject or conflict state — never collecting past review. Dirt or ambiguity in the lane is
...
in-force task-scoped user grant authorizes landing under its stated conditions. When landing is
authorized and neither the user nor repository guidance names a method, default to squash. Landing
evidence applies only to the reviewed identity; a changed subject first needs a new review result.
