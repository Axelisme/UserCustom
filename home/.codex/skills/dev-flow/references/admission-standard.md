...
## S4 — Cycle admission (Root, at every correction)

- **S4.1** Correction Waves for one Slice: **≤ 2**.
- **S4.2** The third is not an implementation event, it is a **scope event**. Exactly three actions are legal: shrink the Slice (return to S1), downgrade the remaining findings to backlog, or report to the user. Opening another correction Wave on the same Slice is not among them.
...
- **S5.1** The user exercises the S1.1 sentence against the real entrypoint and accepts or rejects it. This happens **per Slice**, not once per effort.
- **S5.2** Rejection returns the Slice to S1. It does not enter a correction cycle: the sentence was wrong, and no amount of correction fixes a wrong sentence.
