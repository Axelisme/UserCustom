...
當前任務有 durable task record 時,啟用當下往其 `INDEX.md` 的 `## Standing orders` 補一行:
**逐字引用** user 的觸發語、標記時間、指向本 skill,失效點寫「逐條報告提交或 user 撤回」。
**不要複述**本 skill 的授權與排除清單——那些留在這裡,那一行只記「什麼在生效」。
這是本模式活過 compaction 的唯一憑據:觸發語會被壓縮沖掉,而 `dictator-log.md` 記的是你的
決策,不是「模式仍生效」這個事實。
...
- 多 agent 並行時,遵守 collab 的 one-writer 與 writer-placement guidance;動工前宣告 write
  scope,同檔案/同 public API 的工作不並行。
