...
Root records the validation mode and one specific reason here; the ticket carries both. `Validation
mode: TDD — <specific red-first leverage>` when a reasonable production red oracle exists and
writing it first materially reduces uncertainty in the admitted behavior. The worker commits
Contract tests, fixtures, and test adapters before implementation, records focused red evidence, and
keeps Contract paths protected by normalized `Immutable:` trailers. `Validation mode: direct —
<specific reason>` when no reasonable production red oracle exists or a red-first test would not
reduce uncertainty in the admitted behavior; the delivery is validated against a frozen acceptance
surface without fabricating Contract tests, fixtures, red evidence, or `Immutable:` trailers. File
type never selects the mode. Execution that changes the mode judgment stops and returns to Root for
...
### S2.0 — Uncertain-test lifecycle

Admission owns the uncertain-test lifecycle. When a TDD Contract needs a provisional test to reduce
red-first uncertainty, place it under the ticket-scoped
`<existing-test-root>/_probe/<ticket-id>` path and keep it unchanged through first green. Collection refuses before repository
mutation when the final current-integration-to-lane diff still adds tracked files with the exact case-sensitive `_probe` path
component; the refusal reports deterministic paths and tells Root to promote or delete them. A
probe added and removed, promoted out of `_probe`, or merely modified after it already exists in
integration is not residual. After first green, promote useful public-seam coverage into the repository's
ordinary/canonical test structure in an independent amendment commit; do not add a resolution command or weaken the frozen
acceptance surface. Runtime bindings point here rather than restating this lifecycle.
