...
integration branch's first-parent history and open its exact `acceptance-open` ref. A second start,
including the same SHA, refuses until Root records `acceptance result --verifier agent --outcome
pass|fail`. Canonical tests and ReviewGate run there. Agent PASS/FAIL requires a clean detached HEAD
equal to the open ref and atomically closes it while moving or preserving `accepted`; user PASS/FAIL
records only independent `user-accepted` evidence and leaves the agent gate open. A persistence
advance routes through an admitted writer lane and `integration reconcile`; after collection the exact
snapshot must receive agent acceptance again before it can become landed; external waits do not grant
lifecycle authority.
...
as `{sha, comment?, uncollected}` objects with top-level `pending`. When an agent gate is open,
status also projects `acceptance_open` with the exact SHA and actionable PASS/FAIL warning; the gate
never blocks `lane create`. `uncollected` is Git-derived first-parent work outside integration;
