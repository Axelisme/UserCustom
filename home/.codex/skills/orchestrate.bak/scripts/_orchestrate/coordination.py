...
from .telemetry import acceptance_open_state, lane_comments, record_event, write_report
...
def status(
    ...
    *,
    ...
    acceptance_head = acceptance_state["head"] if isinstance(acceptance_state["head"], str) else None
    acceptance_open = acceptance_open_state(task, acceptance_head)
    if acceptance_open is not None:
        data["acceptance_open"] = acceptance_open
        data.setdefault("warnings", []).append(
            f"agent acceptance awaits explicit PASS or FAIL for {acceptance_open}; "
            "next action: acceptance result --verifier agent --outcome pass|fail"
        )
    if projection.warnings:
        data.setdefault("warnings", []).extend(projection.warnings[:20])
    if (
        include_step
        or projection.pending > 0
        or any(lane.uncollected > 0 for lane in projection.lanes.values())
        or bool(projection.warnings)
        or acceptance_open is not None
...
def _residual_probe_paths(
    repo: RepositoryContext, integration_tip: str, lane_tip: str
) -> list[str]:
    """Find newly added exact ``_probe`` components in the final lane diff."""
    return [
        path
        for path in added_paths(repo.worktree_root, integration_tip, lane_tip)
        if "_probe" in PurePosixPath(path).parts
    ]
...
def integration_collect(
    ...
    integration_tip = _ref(repo, task.integration_branch)
    assert integration_tip is not None
    residual_probe_paths = _residual_probe_paths(repo, integration_tip, lane_tip)
    if residual_probe_paths:
        joined = ", ".join(residual_probe_paths)
        return CommandResult(
            False,
            {
                "code": "unresolved_probe_paths",
                "message": f"integration collect found unresolved probe paths: {joined}",
                "details": {"paths": residual_probe_paths},
                "repair": "Promote the probe paths into canonical tests or delete them before collecting the lane.",
            },
        )
    merge = run_git(
...
def integration_remove(
    ...
    *,
    ...
    acceptance_inventory = worktree_state(repo, task.acceptance_path)
    acceptance_head = acceptance_inventory["head"] if isinstance(acceptance_inventory["head"], str) else None
    acceptance_open = acceptance_open_state(task, acceptance_head)
    if acceptance_open is not None and not abandon:
        _error(
            f"task has an open agent acceptance gate at {acceptance_open}",
            "task_incomplete",
            "Record acceptance result with --verifier agent and --outcome pass or fail before removing the task.",
        )
    if not abandon:
        integration_state = worktree_state(repo, task.integration_path)
        if (
    ...
    for ref in [
        ...
        *(lane.base_ref for lane in lanes),
        ...
        present = (
            ...
            if ref == task.acceptance_open_ref
            else _ref(repo, ref)
        )
