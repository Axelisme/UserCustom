...
from .telemetry import acceptance_open_state, record_event
...
def _open_acceptance_error(subject: str) -> OrchestrateError:
    return OrchestrateError(
        f"agent acceptance gate is open at {subject}; record acceptance result "
        "with --verifier agent and --outcome pass or fail before starting another",
        "acceptance_open",
        "Run acceptance result with --verifier agent and --outcome pass or fail.",
    )
...
def acceptance_start(
    ...
    prior_sha = prior["head"] if isinstance(prior["head"], str) else None
    open_sha = acceptance_open_state(task, prior_sha)
    if open_sha is not None:
        raise _open_acceptance_error(open_sha)
    ...
    run_git(repo.worktree_root, "update-ref", task.acceptance_open_ref, subject)
...
def _acceptance_ref_transaction(
    repo: RepositoryContext,
    task: TaskResources,
    *,
    outcome: str,
    ...
    """Move accepted and close the gate in one Git ref transaction."""
    # Probe the raw ref type in Git's non-dereferencing transaction path before
    # queueing any authority mutation. A post-validation symbolic interleave is
    # therefore rejected without allowing update-ref to follow its target.
    run_git(
        repo.worktree_root,
        "update-ref",
        "--stdin",
        ...
        input_text=(
            "start\n"
            "option no-deref\n"
            f"symref-verify {task.acceptance_open_ref} {task.accepted_ref}\n"
            "prepare\n"
            "abort\n"
        ),
    )
    ...
    if symbolic.returncode == 0:
        raise OrchestrateError(
            "update-ref refused a symbolic acceptance gate during the authority transaction",
            "git_error",
        )
    if symbolic.returncode != 1:
        detail = symbolic.stderr.strip() or symbolic.stdout.strip()
        raise OrchestrateError(
            f"update-ref could not inspect the acceptance gate: {detail}",
            "git_error",
        )
    ...
    if outcome == "pass":
        if previous != head:
            old = previous or ("0" * len(head))
            commands.append(f"update {task.accepted_ref} {head} {old}")
        commands.append(f"delete {task.acceptance_open_ref} {open_subject}")
    else:
        if previous == head:
            commands.append(f"delete {task.accepted_ref} {head}")
        commands.append(f"delete {task.acceptance_open_ref} {open_subject}")
    ...
    if result.returncode:
        ...
        raise OrchestrateError(
            f"acceptance authority transaction failed: {detail}", "git_error"
        )
...
def acceptance_result(
    ...
    explicit_open = acceptance_open_state(task)
    open_subject = acceptance_open_state(task, head) if verifier == "agent" else explicit_open
    ...
    if verifier == "agent":
        if open_subject is None:
            raise OrchestrateError(
                "agent acceptance gate is not open; start acceptance before recording "
                "an agent PASS or FAIL",
                "acceptance_open",
                "Run acceptance start, then record an agent PASS or FAIL.",
            )
        if open_subject != head:
            raise OrchestrateError(
                f"agent acceptance gate at {open_subject} does not match acceptance "
                f"workspace HEAD {head}",
                "acceptance_open",
                "Restore the open acceptance snapshot, then record agent PASS or FAIL.",
            )

    # A legacy unmatched start has no ref. Materialize it only after the exact
    # acceptance subject has passed all read-only validation.
    if verifier == "agent" and explicit_open is None:
        run_git(repo.worktree_root, "update-ref", task.acceptance_open_ref, head)
        assert open_subject == head
...
def integration_land(
    ...
    if landed == accepted:
        current_integration = _authority_ref(
            repo,
            f"refs/heads/{task.integration_branch}",
            label="integration branch authority slot",
        )
        if (
            acceptance_open_state(task) is not None
            and current_integration is not None
            and current_integration != accepted
        ):
            return CommandResult(
                True,
                {},
                ("accepted snapshot lags current integration",),
            )
        raise OrchestrateError(
