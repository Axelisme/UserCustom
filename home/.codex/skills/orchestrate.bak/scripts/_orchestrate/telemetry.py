...
from .git_ops import direct_commit_ref, run_git
...
def _legacy_pending_starts(task: TaskResources) -> list[dict[str, Any]]:
    """Replay historical acceptance events without changing their bytes."""
    try:
    ...
    for event in events:
        ...
        if operation == "acceptance-start" and event.get("outcome") in {"success", "superseded"} and isinstance(subject, str):
            pending.append(event)
            continue
        if operation != "acceptance-result" or event.get("outcome") not in {"pass", "fail"} or not isinstance(subject, str):
            continue
        verifier = event.get("verifier")
        # v180 wrote verifier, while hand-authored/older telemetry may omit it;
        # an omitted verifier is the historical agent stream, not user evidence.
        if verifier is not None and verifier != "agent":
            continue
        for index, start in enumerate(pending):
            if start.get("subject_sha") == subject:
                pending.pop(index)
                break
    return pending


def acceptance_open_state(task: TaskResources, current_head: str | None = None) -> str | None:
    """Return the explicit gate, or the latest recoverable legacy gate."""
    explicit = direct_commit_ref(
        task.repo.worktree_root,
        task.acceptance_open_ref,
        label="acceptance-open authority slot",
    )
    if explicit is not None:
        return explicit
    if current_head is None:
        return None
    pending = _legacy_pending_starts(task)
    if not pending:
        return None
    latest = pending[-1].get("subject_sha")
    return latest if latest == current_head else None
...
def compute_report(
    ...
    counts = {
        ...
        "acceptance_starts": 0,
        "acceptance_agent_passes": 0,
        "acceptance_agent_failures": 0,
        "acceptance_superseded": 0,
        "acceptance_unresolved": 0,
        "acceptance_user_passes": 0,
        "acceptance_user_failures": 0,
    ...
    acceptance_starts: list[tuple[str, datetime, str]] = []
    acceptance_duplicate_subjects: set[str] = set()
    acceptance_orphan_subjects: set[str] = set()
    acceptance_user_markers: list[dict[str, object]] = []
    task_start: tuple[datetime, str] | None = None
    report_warnings = list(snapshot.get("warnings", []))
    modern_acceptance = any(
        event.get("operation") == "acceptance-result" and "verifier" in event
        for event in events
    ) or any(
        isinstance(event.get("orchestrate_version"), int)
        and int(event["orchestrate_version"]) >= 181
        for event in events
    )
    ...
    for event in events:
        ...
        if operation == "integration-create" and outcome == "success" and task_start is None:
            ...
            counts["acceptance_starts"] += 1
            if outcome == "superseded":
                counts["acceptance_superseded"] += 1
            if any(start[0] == subject for start in acceptance_starts):
                acceptance_duplicate_subjects.add(subject)
            acceptance_starts.append((subject, at, at_text))
        elif operation == "acceptance-result":
            if outcome == "pass":
                counts["acceptance_passes"] += 1
            elif outcome == "fail":
                counts["acceptance_failures"] += 1
            if outcome not in {"pass", "fail"} or not isinstance(subject, str):
                continue
            verifier_value = event.get("verifier")
            verifier = verifier_value if verifier_value in {"agent", "user"} else "agent"
            if verifier == "user":
                if outcome == "pass":
                    counts["acceptance_user_passes"] += 1
                else:
                    counts["acceptance_user_failures"] += 1
                acceptance_user_markers.append(_marker(event, "acceptance-user", subject))
                continue
            if outcome == "pass":
                counts["acceptance_agent_passes"] += 1
            else:
                counts["acceptance_agent_failures"] += 1
            matching_index = next(
                (index for index, start in enumerate(acceptance_starts) if start[0] == subject),
                None,
            )
            if matching_index is None:
                acceptance_orphan_subjects.add(subject)
                continue
            start_subject, start_at, start_text = acceptance_starts.pop(matching_index)
            elapsed = _seconds(start_at, at)
            entry = {
                "type": "span", "kind": "acceptance", "identity": start_subject,
                "outcome": "success" if outcome == "pass" else "fail",
                "started_at": start_text, "ended_at": at_text,
                "elapsed_seconds": elapsed,
            }
            if modern_acceptance:
                entry["verifier"] = "agent"
    ...
    counts["acceptance_unresolved"] = len(acceptance_starts)
    for subject, start_at, start_text in acceptance_starts:
        timeline.append({
            "type": "span", "kind": "acceptance", "identity": subject,
            "outcome": "active", "started_at": start_text,
            "ended_at": now.isoformat(),
            "elapsed_seconds": _seconds(start_at, now),
            **({"verifier": "agent"} if modern_acceptance else {}),
        })
        report_warnings.append(
            f"acceptance unresolved: {subject} started at {start_text}"
        )
    for subject in sorted(acceptance_duplicate_subjects):
        report_warnings.append(
            f"acceptance legacy ambiguity: multiple unmatched starts for {subject}"
        )
    for subject in sorted(acceptance_orphan_subjects):
        report_warnings.append(f"acceptance orphan agent result: {subject}")
    timeline.extend(acceptance_user_markers)
    ...
    denominators = (
        ...
        ("acceptance_verdict_coverage", counts["acceptance_starts"] - counts["acceptance_unresolved"], counts["acceptance_starts"]),
        ("acceptance_agent_pass_rate", counts["acceptance_agent_passes"], counts["acceptance_agent_passes"] + counts["acceptance_agent_failures"]),
    ...
    if not modern_acceptance:
        for key in (
            "acceptance_starts",
            "acceptance_agent_passes",
            "acceptance_agent_failures",
            "acceptance_superseded",
            "acceptance_unresolved",
            "acceptance_user_passes",
            "acceptance_user_failures",
        ):
            counts.pop(key, None)
        rates.pop("acceptance_verdict_coverage", None)
        rates.pop("acceptance_agent_pass_rate", None)
    report: dict[str, Any] = {
        "report_version": 3 if modern_acceptance else 2,
