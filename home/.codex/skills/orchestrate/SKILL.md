...
1. A branch that has not passed targeted tests is not merged. The full suite runs **once** in
   ...
   multi-writer: after all lanes are collected — never both).
2. Critical diffs require a different-identity review. The closed list is **hardware
   operation, persistence/migration, public wire schema, security.**
...
| normal 3–5-slice wave | one domain owner with a pre-authorized queue | green checkpoints run ahead; cumulative/asynchronous review |
| structure-dependent foundation or critical diff | freeze one foundation checkpoint | review-before-next-slice; critical uses a different identity |
...
| dispatch more than one slice, use green run-ahead, a finding ledger, or wave metrics | [Slice queues](references/slice-queues.md) |
...
2. **Freeze.** State objective, acceptance, non-goals, write scope, test seams/oracles, and
   critical axes. Unfrozen contract or ownership is not writer-ready.
...
5. **Verify.** Run targeted evidence, review in proportion to the changed surface, and run
   the broad gate once on the final integrated tree. An aborted or unusable gate is never a
   pass.
...
mechanical → root spot-check; normal → focused review or reasoned root self-review; critical
→ different-identity review.
...
## Slice and green-checkpoint invariant
...
A **green checkpoint** qualifies only at a frozen seam with observed red, the same test plus
affected regression green, an independent oracle across the module interface, non-critical
scope, and no anomaly or retract-class finding. It is **run-ahead evidence, never review
sign-off**. A behavior-dependent next slice may continue; a structure-dependent next slice
waits. Normal green checkpoints need not create per-slice review debt; all critical slices
keep `review-before-next-slice`. Load the slice-queue reference before using this route.
...
current-state cache. Reports and decisions stay in-band. Files carry bulk evidence or durable
narrative, never instructions or workflow signals.

## Definition of done

Design settled; targeted evidence passes; review is proportional to the actual changed
surface; the final integrated tree receives the one broad gate; all task-owned lanes,
worktrees, and temporary artifacts are cleaned; persistence landing (if any) is explicitly
authorized and squashed; decisions and evidence are reported.
