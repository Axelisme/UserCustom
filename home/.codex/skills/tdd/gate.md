...
1. Put the task-local driver and evidence outside the Candidate checkout, or in a path confirmed to
   be ignored. Give each invocation one temporary directory inside an ignored temp area.
2. Fix one Candidate working directory and write the applicable repository commands into the
...
4. Verify that the driver, logs, and invocation temp do not enter the Candidate's Git tree.

Setup is complete when one edited driver names its Candidate directory and commands, its evidence
...
- mode, timestamp, Candidate working directory;
...
operational failure and print its path in the log. Logs remain outside the Candidate so recording
evidence cannot change the subject being measured.

The stop is complete when the caller can identify one Candidate state, the commands and exits that
...
Past the bound, report `STOP_SECONDS` and let the caller open a separate ticket for optimising the
suite. The bound is what puts that decision in front of the caller: each stop pays a little more, no
