...
7. **Tool turns:** When two or more tool calls are independent and no result is needed to formulate another, issue them together with `multi_tool_use.parallel`. Keep model-facing output bounded; write bulky output to an artifact and return its outcome, relevant excerpts, and path.
