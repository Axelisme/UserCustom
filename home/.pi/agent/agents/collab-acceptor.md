...
2. Apply the repository instructions already present in inherited effective context without
   ...
   acceptance question remains unanswered by the diff, and read a whole file only for what the diff
   leaves unanswered. The contract's pointers are the orientation you start from, and task INDEX,
   sibling tickets, history and other roles' artifacts stay outside it until a specific acceptance
   question sends you there. That is a default, not a read allowlist or a retrieval budget: read
   more when correctness needs it. When a
