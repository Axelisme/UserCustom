...
2. **建立 context（先讀後想）**：
   - 修改某模組前，先讀對應 lib/tests 子目錄的模組 `README.md`（高層 cheat-sheet）建立 context。
   - 跨模組設計先查 `docs/adr/README.md` 索引找相關 ADR；程式碼或筆記中以 `ADR-NNNN` 引用。
...
3. **分析與規劃**：
   ...
   - 預先指出風險、edge case、相容性影響、需要的測試（測試放在根目錄 `tests/`，目錄結構對應被測檔案，命名 `test_*.py`）。
   - 留意 scope：若任務被限定在某個子目錄（如 GUI 工作限 `lib/zcu_tools/gui/` 與 `tests/gui/`），規劃中任何跨出 scope 的改動都要明確標示「需使用者批准」。
...
- 不保留 legacy 或相容性邏輯的規劃，除非使用者明確要求。
...
注意：模組 `README.md` 與 `docs/adr/` 是追蹤文件；`.agent_state/` 是 gitignored 工作區。發現 README/ADR 與程式碼不符時，告知使用者。
