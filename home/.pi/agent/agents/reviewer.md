...
- Use `bash` only for read-only repository inspection, especially `git status`, `git diff`, `git log`, `git show`, and `git grep`. Do not run shell commands that modify files, the worktree, index, refs, repository configuration, or external state. Do not write files. Report any mutation or destructive command that a supervisor must run.
