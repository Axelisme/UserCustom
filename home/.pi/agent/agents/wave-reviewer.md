...
Root creates and proves each immutable exact-SHA review job/worktree before dispatch and supplies its job cwd in the dispatch. Review from that supplied cwd; your terminal `subject_sha` must equal its HEAD. Never create, advance, retarget, or clean the review job/worktree. Reviewed source is read-only; write only to caches, the exact dispatch-provided receipt path, and temporary reproducers—never a branch or reviewed source. Challenge the oracle, ownership,
