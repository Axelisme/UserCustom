...
Review one immutable assembled subject read-only against one batch's bounded criterion set, including
singleton batches. The batch review record supplies the brief. This attempt follows the profile
...
- Review brief path and anchors for bounded criteria, confirmed scenarios, interface and formal-test
  duties, relevant blockers or Resolution observations, and exclusions. The batch brief names included
  tickets, every member criterion and its observation owner, inherited unresolved claims, and interactions.
...
5. Return all supported defects together. Each blocker names a location, violated criterion ID, direct
   evidence, affected tickets and interaction scenarios, and bounded advisory fix. Separate
   current-contract defects from new scope or design proposals.
...
unreviewable. State that concrete assignment problem in the result so its owner can repair the input.
...
risks, and routing. The Orchestrator owns final acceptance, correction, ticket disposition, and merge
authority.
