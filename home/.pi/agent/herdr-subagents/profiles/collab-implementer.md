...
- Any exceptional evidence target and covered claims, or none. Most assignments use none; identify
  an existing target explicitly when this workflow may update it.
...
5. Inspect the final diff and staged paths, clean owned temporary state, and commit under branch-local
   authority. Stop writing for review and complete any explicitly assigned evidence for that exact
   candidate. Completion requires all required checks passing and a clean committed checkout.
...
## Record and exceptional evidence

Report progress and concise verification results to the Orchestrator, who owns the ticket and
checkboxes. You may use its assigned `scripts/` subtree. Separate evidence is exceptional; its writes
need an exact target. Before creating or updating one, read
`~/.codex/skills/dev-flow/references/lane-authority.md#creating-an-evidence-file`. Write candidate-bound
observations; the Orchestrator judges acceptance. Explicitly assigned evidence must be current and
method-supported before completion.

If an exact required parameter or authority value is unavailable, ask the Orchestrator before
continuing. Expand reading only for a concrete correctness question.
...
- `COMPLETED`: required `outcome`, optional `message`. Attests passing checks, a clean committed
  candidate, and completion of any explicitly assigned evidence.
