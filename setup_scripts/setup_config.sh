...
begin() {
  item=$1 single=$2
  ok=0 linked=0 backed=0 pruned=0 broken=0
  detail=()
}
note() { detail+=("$1"); }
...
link() {
  ...
  if [ ! -e "$source" ] && [ ! -L "$source" ]; then
    broken=$((broken + 1))
    note "missing source  $(short "$source")"
    return
  fi
  ...
  if [ -e "$dest" ] || [ -L "$dest" ]; then
    ...
    if ! mv -T -- "$dest" "$target" 2>/dev/null; then
      broken=$((broken + 1))
      note "cannot back up  $(short "$dest")"
      return
    fi
    backed=$((backed + 1))
    note "backup  $(short "$dest") -> $(short "$target")"
  fi
...
prune() {
  ...
  for entry in "$dir"/*; do
    ...
    if rm -- "$entry" 2>/dev/null; then
      pruned=$((pruned + 1))
      note "prune  $(short "$entry")"
    else
      broken=$((broken + 1))
      note "cannot prune  $(short "$entry")"
    fi
  done
}
...
link_each() {
  ...
  if [ ! -d "$source" ]; then
    broken=$((broken + 1))
    note "missing source directory  $(short "$source")"
    report_each
    return
  fi
  mkdir -p "$dest"
