...
const [packageEntry, extensionPath, cwd, widthArg, expandedArg, scenarioArg = "standard"] = process.argv.slice(2);
if (!packageEntry || !extensionPath || !cwd || !widthArg || !expandedArg) {
  throw new Error("usage: harness <pi-package-entry> <extension-path> <cwd> <width> <expanded> [scenario]");
}
...
if (scenarioArg === "wrapped-output") {
  args = {
    tool_uses: [
      { recipient_name: "read", parameters: { path: "r-wrap" } },
      { recipient_name: "bash", parameters: { command: "b-wrap" } },
    ],
  };
  content = [
    text(Array.from({ length: 12 }, (_, index) => `READ-WRAP-${String(index + 1).padStart(2, "0")}`).join(" ")),
    text(Array.from({ length: 12 }, (_, index) => `BASH-WRAP-${String(index + 1).padStart(2, "0")}`).join(" ")),
  ];
  details = {
    mode: "concurrent",
    calls: [
  ...
  args = {
    tool_uses: [
      ...
      {
        recipient_name: "grep",
        parameters: { pattern: "unique-failed-input", path: "unique-grep-root" },
      },
