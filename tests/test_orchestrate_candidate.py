...
class ReadyCandidateContractTests(unittest.TestCase):
    """Black-box Contract for the ready-candidate publication lifecycle.

    ``integration create`` builds both the integration worktree and a second,
    detached-HEAD acceptance worktree in one call.  ``integration publish``
    ...
    test artifacts) can never be silently clobbered and a failed publish can
    never leave the candidate ref pointing at a SHA the worktree does not
    ...
    def commit_lane(
        self, worktree: Path, content: str, message: str = "implement",
        *, date: str = "2025-01-01T00:01:00+0000", path: str = "app.txt",
    ) -> str:
    ...
    def collect(self, lane_id: str, sha: str, date: str = "2025-01-01T00:10:00+0000") -> dict[str, Any]:
        return self.payload(self.cli(
    ...
    def publish(self, sha: str) -> subprocess.CompletedProcess[str]:
        return self.cli(
    ...
    def test_integration_create_also_builds_the_acceptance_worktree(self) -> None:
        ...
        self.assertEqual(
            self.git(self.acceptance_path, "rev-parse", "HEAD"), self.base
        )
        # It is a detached-HEAD worktree: no branch, so publish can freely
    ...
    # 2. publish moves the ref and the acceptance worktree HEAD to --sha.
    def test_publish_moves_ref_and_acceptance_head_to_sha(self) -> None:
        self.integration_create()
        target = self.advance_integration("lane-a", "lane a\n", "2025-01-01T00:01:00+0000")

        published = self.payload(self.publish(target))

        self.assertEqual(published["operation"], "integration-publish")
        self.assertEqual(published["sha"], target)
        ...
        self.assertEqual(status["candidate"]["acceptance_worktree"], str(self.acceptance_path))

    # 3. a dirty acceptance worktree refuses publish: the ref keeps its old
    #    value and the worktree's own uncommitted content -- the user's own
    #    leftover test artifacts -- is left completely untouched.
    def test_publish_refuses_a_dirty_acceptance_worktree_and_preserves_user_dirt(self) -> None:
        self.integration_create()
        first = self.advance_integration("lane-a", "lane a\n", "2025-01-01T00:01:00+0000")
        self.payload(self.publish(first))

        (self.acceptance_path / "user-notes.txt").write_text("still testing\n", encoding="utf-8")
        second = self.advance_integration("lane-b", "lane a\nlane b\n", "2025-01-01T00:02:00+0000")

        error = self.error_payload(self.publish(second))
        ...
        self.assertIn("?? user-notes.txt", self.git(self.acceptance_path, "status", "--porcelain"))

    # 4. with no candidate published yet, status reports candidate: null
    #    without erroring.
    def test_status_reports_null_candidate_without_error_when_none_published(self) -> None:
        self.integration_create()
        status = self.status()
    ...
    def test_remove_tears_down_both_worktrees_and_the_candidate_ref(self) -> None:
        self.integration_create()
        target = self.advance_integration("lane-a", "lane a\n", "2025-01-01T00:01:00+0000")
        self.payload(self.publish(target))

        removed = self.payload(self.cli(
            self.root, "integration", "remove", "--root", str(self.root),
            "--task-id", self.task_id,
        ))
