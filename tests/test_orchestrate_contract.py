...
class OrchestrateContractTests(unittest.TestCase):
    def test_release_106_keeps_iron_rules_and_pipeline_byte_exact(self) -> None:
        text = (ORCHESTRATE / "SKILL.md").read_text(encoding="utf-8")
        self.assertIn("skill_version: 106", text)
        iron = section_bytes(text, "## Iron rules", "## Pipeline model")
        pipeline = section_bytes(text, "## Pipeline model", "## Control loop")
        # v106 adds task-id to the dispatch contract so an assignee can self-query the
        # ledger, but the iron and pipeline sections stay byte-exact from v98.
        self.assertEqual(
