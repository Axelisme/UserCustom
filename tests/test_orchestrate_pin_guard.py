...
class VerifiedReleaseTaskMutationGuardTests(unittest.TestCase):
        ...
        for name, arguments in cases.items():
            with self.subTest(command=name):
                ...
                try:
                    ...
                    self.assertEqual(payload["orchestrate_version"], 136)
        ...
        try:
            ...
            alternate_skill_md.write_text(
                alternate_skill_md.read_text(encoding="utf-8").replace(
                    "skill_version: 136", "skill_version: 135"
                ),
                encoding="utf-8",
            )
            release = load_release_module()
            (alternate_skill / "manifests/135.json").write_text(
                json.dumps(release.build_manifest(alternate_skill, 135)),
                encoding="utf-8",
            )
            before = self.state(root)
    ...
    def test_missing_and_mismatched_pin_do_not_block_task_mutation(self) -> None:
        ...
        try:
            ...
            self.assertEqual(json.loads(drift.stdout)["orchestrate_version"], 136)
        finally:
            temporary.cleanup()


class CurrentManifestParityTests(unittest.TestCase):
    def test_v136_manifest_is_byte_identical_across_codex_and_pi(self) -> None:
        release = load_release_module()
        self.assertEqual(release.skill_version(CODEX_SKILL), 136)
        codex_manifest = CODEX_SKILL / "manifests/136.json"
        pi_manifest = PI_SKILL / "manifests/136.json"
