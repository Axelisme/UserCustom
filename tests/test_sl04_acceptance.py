...
def run_reviewed_script():
    ...
    snippet = (
        text[text.find("return {") : text.find("return {") + 3000]
        if "return {" in text
        else text[:3000]
    )
    return {
        "hasValidation": has_writer_validation,
        "hasValidationInWorkerSchema": has_validation_word_in_worker_schema,
        "snippet": snippet,
        "text": text,
    }
...
class SL04LeanTerminalTests(unittest.TestCase):
    def test_A1_worker_completed_omits_and_rejects_validation(self):
        ...
        self.assertTrue(
            ok["valid"], "COMPLETED without validation should be valid (lean)"
        )
        ok2 = run_schema(
            {
                "kind": "worker",
                "value": {"outcome": "COMPLETED", "residualRisks": ["r1"]},
            }
        )
        self.assertTrue(ok2["valid"])
        # With validation should be rejected (additionalProperties false)
        bad = run_schema(
            {"kind": "worker", "value": {"outcome": "COMPLETED", "validation": []}}
        )
        ...
        bad2 = run_schema(
            {
                ...
                "value": {
                    "outcome": "COMPLETED",
                    "validation": [{"check": "x", "result": "PASSED", "summary": "y"}],
                    "residualRisks": [],
                },
            }
        )
        ...
        self.assertTrue(
            run_schema(
                {"kind": "worker", "value": {"outcome": "BLOCKED", "blocker": "b"}}
            )["valid"]
        )
        self.assertTrue(
            run_schema(
        ...
        self.assertTrue(
            run_schema(
                {
                    "kind": "worker",
                    "value": {"outcome": "COMPLETED", "efficiencyFeedback": "fb"},
                }
            )["valid"]
        )
        self.assertTrue(
    ...
    def test_A1_public_reviewed_omits_validation_preserves_typed_branches(self):
        ...
        text = REVIEWED_LANE.read_text(encoding="utf-8")
        self.assertNotIn(
            "writer.structuredOutput.validation",
            text,
            "REVIEWED must not copy validation",
        )
        ...
        segment = (
            result_text[comp_idx:blocked_idx]
            if comp_idx != -1 and blocked_idx != -1
            else ""
        )
        self.assertNotIn(
            "validation", segment, "COMPLETED branch must not contain validation"
        )
        # Reviewer schema still valid
        self.assertTrue(
            run_schema({"kind": "reviewer", "value": {"verdict": "PASS"}})["valid"]
        )
        self.assertTrue(
            run_schema(
                {
                    ...
                    "value": {
                        ...
                        "blockers": [
                            {"where": "w", "why": "y", "howToFix": "h", "trigger": "t"}
                        ],
                    },
                }
            )["valid"]
    ...
    def test_A2_registered_parameters_unchanged_and_no_evidence_body(self):
        ...
        params_block = (
            text[params_idx : params_idx + 2000] if params_idx != -1 else text[:2000]
        )
    ...
    def test_A5_workflow_provenance_same_target_update_and_fresh_target_preservation(
        self,
    ):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotEqual(
                target1.read_text(encoding="utf-8"), target2.read_text(encoding="utf-8")
            )
